...
const PhotoUploader = () => {
    ...
    return (
        <Form.Group
            className="mb-3 flex-column d-flex justify-content-center align-items-center"
            controlId="coverSelect"
        >
            <img
                src={preview || photo || noPic}
                alt="profile pic"
                className="picuploarder"
            ></img>
            <Form.Control type="file" onChange={uploadPicture} />
            {errors.photo && (
                <Alert variant="danger">{errors.photo.message}</Alert>
            )}
        </Form.Group>
    );
};
