...
export const Work = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      const { data } = await axios.get('http://localhost:5000/techskills', {
  ...
  useEffect(() => {
    if (AICall) {
      ...
      if (!work[AICall - 1].jobtitle) {
        ...
        let getAI = async () => {
          const { data } = await axios.post('http://localhost:5000/ai/completion', promptData, {
            headers: { Authorization: `Bearer ${token}` }
          });
  ...
  return (
    <Fragment>
      <Formik initialValues={{ work }} onSubmit={values => updateProfile(values)}>
        {({ values }) => (
          <Form as={FormikForm}>
                        ...
                        <Accordion.Item key={index} eventKey={index}>
                          <Accordion.Header>
                            {exp.jobtitle
                              ...
                              : `New work experience entry #${index + 1}`}
                          </Accordion.Header>
                          ...
                          <Accordion.Body>
                            ...
                            <Row>
                              <Form.Group className='mb-3' controlId='keyachievements'>
                              ...
                              <Row className='mb-2'>
                                {AIPrompt === 'Please fill up the job title' ? (
                                  <div style={{ color: '#554e57' }} className='mb-2'>
                                    {AIPrompt}
                                  </div>
                                ) : AIPrompt ? (
                                  <>
                                    <div style={{ color: '#554e57' }}>{AIPrompt}</div>
                                    <Button variant='primary' onClick={() => AddToStatement(index)} className='mb-2'>
                                      Add to statement
                                    </Button>
                                  </>
                                ) : null}
                                ...
                                {loader ? (
                                  ...
                                  <>
                                    ...
                                    <Button variant='primary' className='lightbtn' onClick={() => setAICall(index + 1)}>
                                      Prompt AI
                                    </Button>
                                  </>
                                )}
                            ...
                            <Button type='button' className='secondary lightbtn' onClick={() => remove(index)}>
                              Delete
                            </Button>
                          </Accordion.Body>
                        </Accordion.Item>
