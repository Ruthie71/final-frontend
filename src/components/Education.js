...
export const Education = () => {
  ...
  return (
    <Fragment>
      <Formik initialValues={{ education }} onSubmit={values => updateProfile(values)}>
        {({ values }) => (
          <Form as={FormikForm}>
                    ...
                    {values.education.map((ed, index) => (
                      <div className='row' key={index}>
                        <Accordion.Item eventKey={index}>
                          <Accordion.Header>{ed.coursename || `New education entry #${index + 1}`}</Accordion.Header>
                          <Accordion.Body>
                            <Row>
                              <Form.Group className='mb-3' controlId='academicinstitution'>
                                <Form.Label>Name of academic institution</Form.Label>
                                <Form.Control
                                  as={Field}
